...
const Plugin = (props) => {
  ...
  const [FrontAPIToken] = useState("Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJzY29wZXMiOlsicHJpdmF0ZToqIiwic2hhcmVkOioiLCJwcm92aXNpb25pbmciXSwiaWF0IjoxNTgzMTI0NzE2LCJpc3MiOiJmcm9udCIsInN1YiI6ImpwbWQiLCJqdGkiOiI0NGMyYjgxOGNkMGQ2Y2NmIn0.wK8kzIPIZINy4G38-1pY9WANYAcFkCzw_MieJ93RrpY")
  ...
  const [inflowCstData, setinflowCstData] = useState({})
  ...
  useEffect(() => {
    ...
    fetch(`https://ap-southeast-2.aws.webhooks.mongodb-stitch.com/api/client/v2.0/app/reactplugin-uxowx/service/AddressAPI/incoming_webhook/getInflowId?frontId=${frontData.recipient.handle}`, {})
      .then((res) => res.json())
      .then((result) => {
        setinflowId(result[0].inflowId)
        ...
        fetch(
          ...
          .then((res) => res.json())
          .then((result) => {
            setinflowCstData(result);
            console.log("inflowCstData", inflowCstData);
            console.log("frontData", frontData);
            console.log("FrontAvatar", FrontAvatar);
          });
  ...
  }, [FrontAPIToken, FrontAvatar, frontData, inFlowAPI_URL, inflowCstData, orderData])
